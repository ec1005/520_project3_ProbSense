...
class Execute:
    ...
    @classmethod
    def breakties2(self,curr,listofcells,pg, known_grid):
        l=[]
        maxval = -1
        for cell in listofcells:
            val = pg[cell]*(1-fnProb[int(known_grid[cell])])
            #l.append(val)
            if(val>maxval):
                maxval = val
                l = []
    ...
    def reevaluate_target(self,curr,pg, known_grid, agentType=6):
        ...
        return newtarget
    @classmethod
    def reevaluate_target2(self,curr,pg, known_grid, agentType=6):
        '''finds a new target based on probability grid'''
        max_prob=0
        for cell,val in np.ndenumerate(pg):
            if(agentType==7):
        ...
        for cell,val in np.ndenumerate(pg):
            ...
            if(val==max_prob):
                l.append(cell)
        newtarget=self.breakties2(curr,l,pg, known_grid)
